...
Vec3f Room::bound( const Vec3f &v )
{
	Vec3f newV = v;
	float r = 7.5f;
	newV.x = constrain( newV.x, -mDims.x + r, mDims.x - r );
	newV.y = constrain( newV.y, -mDims.y + r, mDims.y - r );
	newV.z = constrain( newV.z, -mDims.z + r, mDims.z - r );
	return newV;
}
